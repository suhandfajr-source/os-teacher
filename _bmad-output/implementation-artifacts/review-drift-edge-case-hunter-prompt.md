# Edge Case Hunter Review Prompt — spec-neon-drift-reconciliation
...
Output Format: Return ONLY a valid JSON array of objects following:
```json
[{
  "location": "file:line",
...
## CLAIMS (from spec)
- Problem: Chain migrasi 19 file ≠ schema.prisma karena commit f0be2d5 masuk Neon via db push tanpa file migrasi.
- Approach: 1 file migrasi rekonsiliasi berisi delta f0be2d5 persis via shadow diff + resolve --applied di Neon + verify:migrations gate script.
- Tasks:
  1. scripts/verify-migrations.mjs + package.json (step 0 baseline run exit 2).
  2. prisma/migrations/20260921000000_reconcile_schedule_prosem_drift/migration.sql.
  3. Neon pre-flight status 19 -> verify empty -> resolve applied -> post-flight status 20 up-to-date.
  4. verify:migrations exit 0.
  5. deferred-work.md settled.
  6. stories/1a-schema-fondasi-migrasi.md SCL record.
