# Review Task: Edge Case Hunter — Story 7 (jalankan di session LLM terpisah)

Read the instruction file below completely and follow it as your review instructions.

claims_file (leave unread until your instructions call for it — Step 5): the spec content inlined near the END of this prompt, under the marker '=== CLAIMS FILE (spec) ==='. Do not scroll to it before Step 5.

Review content: the unified diff inlined below under '=== REVIEW CONTENT (unified diff) ==='.

Do not invoke any skill, and do not spawn subagents of your own — you are the reviewer. Return your findings as text in your final message (JSON array per the instruction file). If the instruction file is unreadable, report that exact failure and stop.

=== INSTRUCTION FILE (edge-case-hunter.md) ===
