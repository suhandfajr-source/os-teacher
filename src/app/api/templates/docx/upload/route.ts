...
function validateSameOrigin(request: NextRequest): boolean {
  ...
  if (!origin || !host) {
    // If origin is missing, check referer host as validated fallback
    const referer = request.headers.get("referer");
    if (!referer) return true; // Direct non-browser request or same-origin without header
    try {
...
export async function POST(request: NextRequest) {
  ...
  // 5. Exactly One File Rule
  if (fileEntries.length === 0) {
    return NextResponse.json(
      { error: "File template dokumen (.docx) wajib diunggah." },
      { status: 400 }
    );
  }
  ...
  // 6. Post-Parse File Size Verification (2 MB limit for file)
  ...
  // 7. Security Preflight & Placeholder Extraction
